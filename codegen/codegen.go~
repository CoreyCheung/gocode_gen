...
func render(tables []model.Table, columns []model.Column, confPrefix string) {
	...
	entitytplPath := viper.GetString(confPrefix + ".entitytpl_path")
	dtotplPath := viper.GetString(confPrefix + ".dtotpl_path")
	servicetplPath := viper.GetString(confPrefix + ".servicetpl_path")
	controllertplPath := viper.GetString(confPrefix + ".controllertpl_path")
	//outputPath := viper.GetString(confPrefix + ".output_path")
	entityoutputPath := viper.GetString(confPrefix + ".entityoutput_path")
	dtooutputPath := viper.GetString(confPrefix + ".dtooutput_path")
	serviceoutputPath := viper.GetString(confPrefix + ".serviceoutput_path")
	controlleroutputPath := viper.GetString(confPrefix + ".controlleroutput_path")
	...
	entitytpl, _ := mustache.ParseFile(entitytplPath)
	dtotpl, _ := mustache.ParseFile(dtotplPath)
	servicetpl, _ := mustache.ParseFile(servicetplPath)
	controllertpl, _ := mustache.ParseFile(controllertplPath)
	fmt.Println("要生成代码了")
	err := os.MkdirAll(entityoutputPath, os.ModePerm)
	if err != nil {
		fmt.Println(err, entityoutputPath)
	}
	err = os.MkdirAll(dtooutputPath, os.ModePerm)
	if err != nil {
		fmt.Println(err)
	}
	err = os.MkdirAll(serviceoutputPath, os.ModePerm)
	if err != nil {
		fmt.Println(err)
	}
	...
	for i := 0; i < len(tableArr); i++ {
		...
		renderAndWriteFile(entitytpl, &table, entityoutputPath, filePrefix+table.OutputFileName(filePostfix)+filePostfix)
		renderAndWriteFile(dtotpl, &table, dtooutputPath, filePrefix+table.OutputFileName(filePostfix)+filePostfix)
		renderAndWriteFile(servicetpl, &table, serviceoutputPath, filePrefix+table.OutputFileName(filePostfix)+filePostfix)
		renderAndWriteFile(controllertpl, &table, controlleroutputPath, filePrefix+table.OutputFileName(filePostfix)+filePostfix)
	}
}
